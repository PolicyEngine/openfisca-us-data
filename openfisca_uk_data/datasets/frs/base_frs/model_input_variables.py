...
def get_input_variables():
    ...
    from openfisca_uk.tools.general import WEEKS_IN_YEAR, where, select, add

    class self_employment_income(Variable):
        value_type = float
        entity = Person
        label = "Income from self-employment. Different to trading profits"
    ...
    class gender(Variable):
        ...
        def formula(person, period, parameters):
            return where(
                person("P_SEX", period) == 1, Gender.MALE, Gender.FEMALE
            )
    ...
    class savings_interest_income(Variable):
        ...
        definition_period = YEAR
        reference = (
            "Income Tax (Trading and Other Income) Act 2005 s. 365(1)(a)"
        )
    ...
    class dividend_income(Variable):
        ...
        definition_period = YEAR
        reference = (
            "Income Tax (Trading and Other Income) Act 2005 s. 365(1)(b-d)"
        )
    ...
    class ESA_contrib_reported(Variable):
        ...
        entity = Person
        label = (
            "Employment and Support Allowance (contribution-based) (reported)"
        )
        definition_period = YEAR
...
def from_BaseFRS(year: int = 2018):
    ...
    for i in range(len(input_variables)):
        ...
        if variable.__name__ in LABOUR_INCOME_VARIABLES:
            ...
            input_variables[i] = uprated(from_year=year + 1)(variable)

    class reform(Reform):
        def apply(self):
            for var in get_FRS_variables():
